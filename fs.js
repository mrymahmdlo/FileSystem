export class Item {
  ...
  set name(newName) {
    if (!newName || typeof newName !== "string" || !newName.trim().length) {
      throw new Error("نام آیتم نباید خالی باشد.");
    }

    if (newName.includes("/")) {
      throw new Error("نام آیتم حاوی نماد نامعتبر است");
    }

    if (this.parent && this.parent.hasItem(newName)) {
      throw new Error(`آیتمی با نام "${newName}" در این مسیر وجود دارد.`);
    }
...
export class File extends Item {
  ...
  get copy() {
    return new File(`${this.name} کپی`, this.textContent, this.source);
  }
}
...
export class Directory extends Item {
  ...
  get copy() {
    const dirCopy = new Directory(`${this.name} کپی`, this.type);
  ...
  insertItem(item) {
    ...
    if (item === this) throw new Error("مسیر نمیتواند شامل خودش باشد.");

    let parent = this.parent;

    while (parent !== null) {
      if (parent === item) {
        throw new Error("مسیر نمی تواند شامل یکی از اجداد خود باشد");
      }
