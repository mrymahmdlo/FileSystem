...
export class MediaDirectory extends FileSystem {
  ...
  init(container) {
    ...
    this.#directories = content.querySelector(
      "#container-wrapper .directories"
    );
  ...
  #render = () => {
    return `
        ...
        <input type="search" placeholder="جست و جو ..." class="search-field"/>
					<div>
						<button type="button" id="go-back-btn">&lt;</button>
						<div id="breadcrumbs" style="display: flex;"></div>
					</div>
					<div>
						<button type="button" id="create-folder-btn" class="primary">پوشه جدید</button>
			...
			<div id="container-wrapper">
				...
				<p class="directory-blank">خالی</p>
			</div>
  ...
  #handleSearchInput = (e) => {
    ...
    if (val.trim().length >= 3) {
      this.#debounceTimer = setTimeout(() => {
        const res = this.findAllItems(
          (item) => item.name.search(new RegExp(val, "g")) >= 0
        );
        this.#searchResults.style.display = "block";
        this.#searchResults.innerHTML = `<h2>نتایج جست و جو<b>(${res.length})</b></h2>`;

        if (res.length > 0) {
          res.forEach((item) => {
            const isFile = item instanceof File;
            const el = document.createElement("li");
            el.innerHTML = `<b>${isFile ? "فایل" : "مسیر"}</b>${item.name}`;
  ...
  #showItemsActions = (e, itemElement, item) => {
    ...
    control.innerHTML = `
			<li class="rename-btn">تغییر نام</li>
			<li class="copy-btn">کپی</li>
			<li class="move-btn">جا به جایی</li>
			<li class="delete-btn">حذف</li>
		`;

    control
      .querySelector(".delete-btn")
      .addEventListener("click", (e) =>
        this.#handleDeleteClick(e, itemElement, item)
      );
    control
      .querySelector(".move-btn")
      .addEventListener("click", (e) => this.#handleMoveItem(e, item));
    control
      .querySelector(".rename-btn")
      .addEventListener("click", (e) =>
        this.#handleRenameItem(e, itemElement, item)
      );
    control
      .querySelector(".copy-btn")
      .addEventListener("click", (e) => this.#handleItemCopy(e, item));

    if (e.type.startsWith("touch")) {
      control.style.left = e.changedTouches[0].clientX + "px";
      control.style.top = e.changedTouches[0].clientY + "px";
    } else {
  ...
  #createFileElement = (file) => {
    ...
    itemElement.innerHTML = `
					...
					<figcaption>
						...
						<p class="file-details"><b>${file.type}</b>: ${
      file.mimeType
    } <span>${bytesToSize(file.source.size)}</span></p>
					</figcaption>
				`;
    itemElement.addEventListener("contextmenu", (e) =>
      this.#showItemsActions(e, itemElement, file)
    );
  ...
  #createFolderElement = (folder) => {
    ...
    itemElement.addEventListener("contextmenu", (e) =>
      this.#showItemsActions(e, itemElement, folder)
    );
  ...
  #renderMediaList = () => {
    ...
    this.#directories.innerHTML = "<h2>مسیر ها</h2>";
    this.#files.innerHTML = "<h2>فایل ها</h2>";
  ...
  #handleAddFolderClick = () => {
    const dialog = new InputDialog("ساخت پوشه", "نام پوشه");
    ...
    dialog.onSave(({ text }) => {
      ...
      if (newDir) {
        ...
        dialog.show(text, `مسیری با نام "${text}" وجود دارد!`);
      }
    });
  ...
  #handleDeleteClick = (e, element, item) => {
    ...
    const dialog = new ConfirmDialog(
      `آیا از حذف مسیر "${item.name}" اطمینان دارید؟`
    );
  ...
  #handleRenameItem = (e, element, item) => {
    ...
    const dialog = new InputDialog("تغییر نام", "نام جدید");
